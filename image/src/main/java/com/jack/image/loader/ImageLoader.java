...
import com.jack.cache.exception.FileLockedException;
import com.jack.cache.file.LruCache;
import com.jack.cache.file.LruDiskCache;
...
import com.jack.image.animation.ImageAnimationHelper;
import com.jack.image.decoder.ImageDecoder;
import com.jack.image.drawerable.AsyncDrawable;
import com.jack.image.drawerable.GifDrawable;
import com.jack.image.drawerable.ReusableDrawable;
 ...
 * 图片加载控制
 */
public final class ImageLoader implements
    ...
    static {
        int memClass = ((ActivityManager) ImageManagerImpl.getContext()
    ...
    private ImageLoader() {
        if (imageManager == null) {
            imageManager = ImageManagerImpl.getInstance(null);
        }
    }
    ...
    static Cancelable doLoadFile(final String url,
        ...
        return HttpManagerImpl.getInstance(null).get(params, callback);
    }
    ...
    private Cancelable doLoad(ImageView view,
        ...
        return cancelable = HttpManagerImpl.getInstance(null).get(params, this);
    ...
    public void onError(Throwable ex, boolean isOnCallback) {
        ...
        if (ex instanceof FileLockedException) {
            ...
            imageManager.postDelayed(new Runnable() {
                @Override
                public void run() {
    ...
    private static void postArgsException(
        ...
        ImageManagerImpl.getInstance(null).autoPost(new Runnable() {
            @Override
            public void run() {
                try {
                    if (callback instanceof ProgressCallback) {
    ...
    private final static class FakeImageView extends ImageView {
        ...
        public FakeImageView() {
            super(ImageManagerImpl.getContext());
        }
