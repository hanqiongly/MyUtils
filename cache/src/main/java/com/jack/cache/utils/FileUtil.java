...
public class FileUtil {
    ...
    public static File getCacheDir(String dirName) {
        File result;
        if (existsSdcard()) {
            File cacheDir = DbManagerImpl.getContext().getExternalCacheDir();
            if (cacheDir == null) {
                result = new File(Environment.getExternalStorageDirectory(),
                        "Android/data/" + DbManagerImpl.getContext().getPackageName() + "/cache/" + dirName);
            } else {
                result = new File(cacheDir, dirName);
            }
        } else {
            result = new File(DbManagerImpl.getContext().getCacheDir(), dirName);
        }
