package com.jack.cache.file.lock;
...
public final class ProcessLock implements Closeable {
    ...
    static {
        File dir = DbManagerImpl.getContext().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE);
    ...
    private static ProcessLock tryLockInternal(final String lockName, final String hash, final boolean writeMode) {
        synchronized (LOCK_MAP) {
            ...
            try {
                File file = new File(
                        DbManagerImpl.getContext().getDir(LOCK_FILE_DIR, Context.MODE_PRIVATE),
                        hash);
