package com.jack.http.request.param;

import android.text.TextUtils;

import com.jack.executor.utils.LogUtil;
import com.jack.http.data.HttpKeyValue;
import com.jack.http.request.HttpMethod;
import com.jack.http.request.body.BodyItemWrapper;
import com.jack.http.request.body.FileBody;
import com.jack.http.request.body.InputStreamBody;
import com.jack.http.request.body.MultipartBody;
import com.jack.http.request.body.RequestBody;
import com.jack.http.request.body.StringBody;
import com.jack.http.request.body.UrlEncodedParamsBody;
...
public abstract class BaseParams {
    ...
    private final List<HttpKeyValue> queryStringParams = new ArrayList<HttpKeyValue>();
    private final List<HttpKeyValue> bodyParams = new ArrayList<HttpKeyValue>();
    private final List<HttpKeyValue> fileParams = new ArrayList<HttpKeyValue>();
        ...
        if (method == null || HttpMethod.permitsRequestBody(method)) {
            if (!TextUtils.isEmpty(name)) {
                if (value instanceof File
                    ...
                    this.fileParams.add(new HttpKeyValue(name, value));
                } else {
                    if (value instanceof List) {
                        ...
                        this.bodyParams.add(new HttpKeyValue(name, value));
                    }
                }
            } else {
            ...
            if (!TextUtils.isEmpty(name)) {
                if (value instanceof List) {
                    ...
                    this.queryStringParams.add(new HttpKeyValue(name, value));
                }
            }
        }
    }
    ...
    public void addBodyParameter(String name, Object value, String contentType, String fileName) {
        if (TextUtils.isEmpty(contentType) && TextUtils.isEmpty(fileName)) {
            ...
            this.fileParams.add(new HttpKeyValue(name, new BodyItemWrapper(value, contentType, fileName)));
        }
    }
    ...
    public List<HttpKeyValue> getQueryStringParams() {
        checkBodyParams();
        return new ArrayList<HttpKeyValue>(queryStringParams);
    }

    public List<HttpKeyValue> getBodyParams() {
        checkBodyParams();
        return new ArrayList<HttpKeyValue>(bodyParams);
    }

    public List<HttpKeyValue> getFileParams() {
        checkBodyParams();
        return new ArrayList<HttpKeyValue>(fileParams);
    }

    public List<HttpKeyValue> getStringParams() {
        List<HttpKeyValue> result = new ArrayList<HttpKeyValue>(
    ...
    public String getStringParameter(String name) {
        for (HttpKeyValue kv : queryStringParams) {
        ...
        for (HttpKeyValue kv : bodyParams) {
            if (name == null && kv.key == null) {
                return kv.getValueStr();
            } else if (name != null && name.equals(kv.key)) {
                return kv.getValueStr();
    ...
    public List<HttpKeyValue> getParams(String name) {
        List<HttpKeyValue> result = new ArrayList<HttpKeyValue>();
        for (HttpKeyValue kv : queryStringParams) {
            if (name == null && kv.key == null) {
                result.add(kv);
        ...
        for (HttpKeyValue kv : fileParams) {
            if (name == null && kv.key == null) {
                result.add(kv);
            } else if (name != null && name.equals(kv.key)) {
                result.add(kv);
    ...
    public void removeParameter(String name) {
        if (!TextUtils.isEmpty(name)) {
            Iterator<HttpKeyValue> it = queryStringParams.iterator();
            while (it.hasNext()) {
                HttpKeyValue kv = it.next();
                if (name.equals(kv.key)) {
                    it.remove();
    ...
    public String toJSONString() {
        List<HttpKeyValue> list = new ArrayList<HttpKeyValue>(queryStringParams.size() + bodyParams.size());
    ...
    private void params2Json(final JSONObject jsonObject, final List<HttpKeyValue> paramList) throws JSONException {
        ...
        for (int i = 0; i < paramList.size(); i++) {
            HttpKeyValue kv = paramList.get(i);
    ...
    public final static class ArrayItem extends HttpKeyValue {
        public ArrayItem(String key, Object value) {
            super(key, value);
        }
    }

    public final static class Header extends HttpKeyValue {
